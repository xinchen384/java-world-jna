...
object ConsumerPerformance {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val config = new ConsumerPerfConfig(args, 1)
    val config1 = new ConsumerPerfConfig(args, 2)
    ...
    val secMessagesRead = new AtomicLong(0)
    val secBytesRead = new AtomicLong(0)
    val secTimeout = new AtomicBoolean(false)
    ...
    if(config.useNewConsumer) {
      ...
      threadList ::= new ConsumerThread(1, "consumerThread1", consumer, List(config.topic), config.numMessages, 100000, config, totalMessagesRead, totalBytesRead)

      val consumer1 = new KafkaConsumer[Array[Byte], Array[Byte]](config1.props)
      consumer1.subscribe(List(config1.topic))
      threadList ::= new ConsumerThread(2, "consumerThread2", consumer1, List(config.topic), config.numMessages, 100000, config1, totalMessagesRead, totalBytesRead)
      ...
      for (thread <- threadList)
        thread.join
      //consume(consumer, List(config.topic), config.numMessages, 100000, config, totalMessagesRead, totalBytesRead)
      ...
      // the second consumer group
      //val consumerConfig1 = new ConsumerConfig(config1.props)
      //val consumerConnector1: ConsumerConnector = Consumer.create(consumerConfig1)
      //val topicMessageStreams1 = consumerConnector1.createMessageStreams(Map(config1.topic -> config1.numThreads))
      //for ((topic, streamList) <- topicMessageStreams1)
      //  for (i <- 0 until streamList.length)
      //    threadList ::= new ConsumerPerfThread(i+1, "kafka-zk-consumer-" + (i+1), streamList(i), config1, secMessagesRead, secBytesRead, secTimeout)
    ...
    while(messagesRead < count && System.currentTimeMillis() - lastConsumedTime <= timeout) {
      ...
      for(record <- records) {
        ...
        if(record.value != null){
          ...
          println("received content: " + record.value) 
        }
      }
  ...
  def printProgressMessage(id: Int, bytesRead: Long, lastBytesRead: Long, messagesRead: Long, lastMessagesRead: Long,
    ...
    println(("%s, threadId: %d, %.4f, %.4f, %d, %.4f").format(dateFormat.format(endMs), id, totalMBRead,
        1000.0 * (mbRead / elapsedMs), messagesRead, ((messagesRead - lastMessagesRead) / elapsedMs) * 1000.0))
  }

  class ConsumerPerfConfig(args: Array[String], groupId: Int) extends PerfConfig(args) {
    ...
    val groupIdOpt = parser.accepts("group", "The group id to consume on.")
      ...
      .defaultsTo("perf-consumer-" + groupId)
      .ofType(classOf[String])
    ...
    override def run() {
      ...
      try {
        ...
        while (iter.hasNext && messagesRead < config.numMessages) {
          ...
          println("received message: " + messageAndMetadata.message)
  ...
  class ConsumerThread
    (threadId: Int, name: String, consumer: KafkaConsumer[Array[Byte], Array[Byte]], topics: List[String], 
    count: Long, timeout: Long, config: ConsumerPerfConfig, totalMessagesRead: AtomicLong, totalBytesRead: AtomicLong)
    extends Thread(name) {
    ...
    println("consumer: " + name + " receives "+ messagesRead)
